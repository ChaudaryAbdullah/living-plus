...
const PaymentRenter = () => {
  ...
  const fetchUser = async () => {
    try {
      ...
      if (response.data?.user?.tenantId) {
        ...
        setError(
          "Tenant ID not found. Please ensure you're logged in as a tenant."
        );
      }
  ...
  const fetchBills = async (tenantId) => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/payment/tenant/${tenantId}`,
        {
          withCredentials: true,
        }
      );
  ...
  const handlePrintBill = (id) => {
    ...
    if (billToPrint) {
      ...
      pdf.text(
        15,
        90,
        `Due Date: ${new Date(billToPrint.dueDate).toLocaleDateString()}`
      );
  ...
  const handlePayBill = async (paymentId) => {
    try {
      ...
      const { data: existingPayment } = await axios.get(
  ...
  const filteredBills = bills.filter((bill) => {
    return (
      ...
      (statusFilter === "" ||
        bill.status.toLowerCase().includes(statusFilter.toLowerCase())) &&
